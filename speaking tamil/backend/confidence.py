...
def analyze_confidence(audio_path: str, norm_text: str) -> tuple[float, dict[str, Any]]:
    ...
    score = 8.5  # Increased base score (was 7.0)
    
    # Volume stability: lower CV = more stable = higher confidence
    if vol_cv < 0.5:
        score += 1.0  # Very stable (reduced from 1.5)
    elif vol_cv < 1.0:
        score += 0.5  # Moderately stable
    elif vol_cv > 1.5:
        score -= 0.5  # Unstable (reduced penalty from 1.0)
    
    # Few drop-offs = confident delivery
    if drop_ratio < 0.2:
        score += 0.5  # Reduced from 1.0
    elif drop_ratio > 0.4:
        score -= 0.5  # Many hesitations (reduced from 1.5)
    ...
    if pitch_std > 0:
        if 15.0 <= pitch_std <= 50.0:
            ...
            score -= 0.3  # Too monotone (reduced from 1.0)
        elif pitch_std > 70.0:
            score -= 0.3  # Too erratic (reduced from 1.0)
    
    # Penalty for fillers - more lenient
    if filler_count > 8:  # Increased threshold from 5
        score -= min(1.0, (filler_count - 8) * 0.2)  # Reduced penalty
